...
if(isset($_GET['policy_id'])){
    ...
    $term_id = $_GET['policy_id'];
    
    $term_query = "select * from policy where policy_id='$term_id'";
    
    $term_edit = mysqli_query($con,$term_query);
    
    $term_edit = mysqli_fetch_array($term_edit);
    
    $term_id = $term_edit['policy_id'];
    
    $term_title = $term_edit['policy_title'];
    
    $term_desc = $term_edit['policy_desc'];

    $term_link=$term_edit['policy_link'];

    $term_status=$term_edit['policy_status'];
}
$error_tiitle="";
$error_desc="";
$error_link=""; 
...
if(isset($_POST['update'])){
    ...
    $update_term = "update policy set policy_title='$term_title',policy_link='$term_link',policy_desc='$term_desc',policy_status='$term_statuss' where policy_id='$term_id'";
    ...
    if($run_term){
      ...
      .then((willDelete) => {
              if (willDelete) {
                  window.open('view-policy.php','_self');
              } 
              else {
              }
      });
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-flex align-items-center justify-content-between">
                    <h4 class="mb-0 font-size-18">Update Policy</h4>
                </div>
            </div>
        </div>     
        <!-- end page title -->
                    ...
                    <div class="card-body">
                       <form class="custom-validation" method="POST" enctype="multipart/form-data"> 
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <input class="form-control" type="text" placeholder="Policy Title " name="term_title" value="<?php echo $term_title; ?>" id="example-text-input" required>
                                    <span style="color: red;"><?php echo $error_tiitle; ?></span>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="example-text-input" class="col-md-3 col-form-label">Policy Link</label>
                                <div class="col-md-9">
                                    <input class="form-control" type="text" placeholder="Policy Link" name="term_link" value="<?php echo $term_link; ?>" id="example-text-input" required>
                                    <span style="color: red;"><?php echo $error_link; ?></span>
                                </div>
                            </div>
                            
                            <div class="form-group row">
                                <label for="example-number-input" class="col-md-3 col-form-label">Policy Desc</label>
                                <div class="col-md-9">
                                <textarea  required class="form-control" placeholder="Policy Desc" name="term_desc" cols="19" rows="6" ><?php echo $term_desc; ?></textarea>
                                <span style="color: red;"><?php echo $error_desc; ?></span>
                            </div>
                            </div>
                            <div class="form-group row">
                            <label for="example-text-input" class="col-md-3 col-form-label">Policy Status</label>
                                <?php
                                    if($term_status=="no")
                                    { 
                                ?>
...
$(document).ready(function(){  
 ...
 { 
     if(counter<=30)
     {
                ...
                .then((willDelete) => {
                        if (willDelete) {
                            window.open('update-policy.php','_self');
                        } 
                        else
                        {
