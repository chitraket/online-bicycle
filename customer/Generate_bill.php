...
$pdf->Cell(30	,5,'Product id',1,0);
$pdf->Cell(80	,5,'Product Name',1,0);
...
while ($row_c_orders = mysqli_fetch_array($run_c_orders)) {
    ...
    if ($pro_papage==0) {
        ...
        while ($row_products=mysqli_fetch_array($run_products)) {
            ...
            $pdf->Cell(30, 5, "{$pro_id}", 1, 0);
            $pdf->Cell(80, 5, "{$product_title}", 1, 0);
            $pdf->Cell(25, 5, "{$product_price}", 1, 0);
            $pdf->Cell(20, 5, "{$pro_qty}", 1, 0);
            $pdf->Cell(30, 5, "{$sub_total}", 1, 1, 'R');
        }
    }
    if ($pro_papage==1) {
        ...
        while ($row_accessories=mysqli_fetch_array($run_accessories)) {
          ...
          $accessories_prices=$row_accessories['accessories_prices'];
            ...
            $pdf->Cell(30, 5, "{$pro_id}", 1, 0);
            $pdf->Cell(80, 5, "{$accessories_name}", 1, 0);
